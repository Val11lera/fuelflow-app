...
function money(n: number, c: string) { return `${sym(c)}${round2(n).toFixed(2)}`; }

// Draw multiple lines without affecting global flow; returns endY
function drawLines(doc: any, lines: string[], x: number, y: number, width: number) {
  const lh = doc.currentLineHeight();
  for (const line of lines) {
    doc.text(line, x, y, { width, lineBreak: false });
    y += lh;
  }
  // reset cursor so PDFKit won't flow
  doc.x = x; doc.y = 0;
  return y;
}
...
export async function buildInvoicePdf(payload: InvoiceInput): Promise<BuiltInvoice> {
  ...
  /* From / Bill To */
  const leftX = MARGIN, rightX = pageW/2 + 12, colW = pageW/2 - MARGIN - 24;
  let y = 100;

  doc.font("Helvetica-Bold").fontSize(11)
     .text("From", leftX, y, { lineBreak: false })
     .text("Bill To", rightX, y, { lineBreak: false });
  ...
  const leftEndY  = drawLines(doc, leftLines,  leftX,  y+16, colW);
  const rightEndY = drawLines(doc, rightLines, rightX, y+16, colW);
  ...
  /* Build rows (unitPrice is per litre) */
  type Row = { description: string; qty: number; unitEx: number; net: number; vatPct: number; vat: number; gross: number; };
  const rows: Row[] = [];
  ...
  payload.items.forEach(it => {
    ...
    rows.push({ description: it.description || "Item", qty, unitEx, net, vatPct: VAT_ENABLED ? VAT_RATE*100 : 0, vat, gross });
  });
  ...
  // rows
  let rowY = y + 24;
  const rowH = 22, bottomSafe = pageH - 150;
  ...
  for (let i=0;i<rows.length;i++){
    ...
    if (rowY + rowH > bottomSafe) {
      ...
      doc.font("Helvetica-Oblique").fontSize(9).fill("#6B7280")
         .text(`(+${remaining} more item${remaining>1?"s":""} not shown)`, tableX+10, rowY+6, { width: tableW-20, lineBreak: false });
      rowY += rowH; break;
    }
    ...
    const cells = [
      ...
      { v: fmtQty(r.qty),        w: cols[1].w, align: cols[1].align },
      { v: money(r.unitEx,currency), w: cols[2].w, align: cols[2].align },
      { v: money(r.net,currency),    w: cols[3].w, align: cols[3].align },
      { v: VAT_ENABLED ? `${r.vatPct.toFixed(0)}%` : "—", w: cols[4].w, align: cols[4].align },
      { v: VAT_ENABLED ? money(r.vat,currency) : money(0,currency), w: cols[5].w, align: cols[5].align },
      { v: money(r.gross,currency),  w: cols[6].w, align: cols[6].align },
    ] as const;
  ...
  const totals = [
    { label: "Subtotal (Net)", value: money(netTotal,currency), bold: false },
    { label: "VAT",            value: money(vatTotal,currency), bold: false },
    { label: "Total",          value: money(grandTotal,currency), bold: true },
  ];
  for (let i=0;i<totals.length;i++){
    const t = totals[i], h = 22;
  ...
  if (payload.meta?.notes) {
    ...
    // manually split long notes to avoid wrapping flow
    const words = payload.meta.notes.split(/\s+/);
    const maxChars = 90;
    const lines: string[] = [];
    let line = "";
    ...
    rowY = drawLines(doc, lines, tableX, rowY+14, pageW - MARGIN*2);
  }

  /* Footer (same page, absolute) */
  doc.x = MARGIN; doc.y = 0; // keep engine cursor high so no auto-page
  doc.font("Helvetica").fontSize(9).fill("#6B7280").text(
    `${companyName} — Registered in England & Wales${companyNo ? " • Company No " + companyNo : ""}${companyVat ? " • VAT No " + companyVat : ""}`,
    MARGIN, pageH - 40, { width: pageW - MARGIN*2, align: "center", lineBreak: false }
  );

  doc.end();
  await done;

  return { pdfBuffer: Buffer.concat(chunks), filename: `${invNo}.pdf`, total: grandTotal };
}
