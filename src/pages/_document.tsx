...
export default function DocumentsPage() {
  ...
  const badge = (tone: "ok" | "warn" | "missing", label: string) =>
    <span className={cx(
      ...
      tone === "missing" && "bg-red-500/15 text-red-300",
    )}>{label}</span>;
  ...
  return (
    <div className={cx(
      ...
      <div className="mt-auto pt-3">
        <a href={cta.href} className="inline-flex items-center rounded-xl bg-white/10 hover:bg-white/15 px-4 py-2 font-semibold">
          {cta.label}
        </a>
      </div>
    </div>
