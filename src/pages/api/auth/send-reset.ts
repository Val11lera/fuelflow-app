...
/** Try to load nodemailer at runtime to avoid build-time type errors on Vercel. */
function getNodemailer() {
  try {
    // eslint-disable-next-line @typescript-eslint/no-var-requires
    return require("nodemailer");
  } catch {
    return null; // not installed (or not available in this env)
  }
}
...
async function verifyHCaptcha(token?: string) {
  ...
  if (!secret) return true; // server verification not configured
    ...
    <body>
      <div class="wrap">
        <div class="card">
          ...
          <div class="footer">
            © FuelFlow • Sent from <a href="${site}" style="color:#ffd54d" target="_blank" rel="noopener">${site.replace(
              /^https?:\/\//,
              ""
            )}</a>
          </div>
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  // If SMTP envs are present, try to send an email via nodemailer (runtime).
  const hasSMTP =
  ...
  if (hasSMTP && nodemailer) {
    try {
      ...
      // If email fails, still return the action link so the client can handle it.
      return res.status(200).json({ ok: true, sent: false, actionUrl, note: "Email send failed" });
    }
  }

  // No nodemailer or SMTP configured: return the link so client can show it / handle alternative flow.
  return res.status(200).json({
    ok: true,
    sent: false,
    actionUrl,
    note: "SMTP/nodemailer unavailable at runtime; returned action link instead.",
  });
}
