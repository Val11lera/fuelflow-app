...
export default async function handler(req: NextApiRequest, res: NextApiResponse<Ok | Fail>) {
  ...
  if (!email || !version) return res.status(400).json({ ok: false, error: 'Missing email/version' });
  ...
  try {
    ...
    return res.status(200).json({ ok: true, accepted: true, id: data.id, accepted_at: data.accepted_at });
  } catch (e: any) {
