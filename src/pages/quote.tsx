...
export default function QuotePage() {
  ...
  const termsUrl = `/terms?source=quote&email=${encodeURIComponent(form.email || "")}`;
  ...
  function validate(current: FormState) {
    ...
    if (!current.quantity_litres || Number(current.quantity_litres) <= 0) e.quantity_litres = "Please enter a valid quantity";
    return e;
  }
  ...
  function focusFirstError(errMap: Record<string, string>) {
    const order = ["customer_name", "email", "phone", "postcode", "quantity_litres"];
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (res.status === 412 || data?.error === "terms_not_accepted") {
        // Show Terms required dialog
    ...
    <div className="min-h-screen text-white relative overflow-x-hidden">
      ...
      <main className="relative z-10 mx-auto w-full max-w-5xl px-4 py-10 md:py-14">
        ...
        {success && lastSummary ? (
          ...
          <form onSubmit={onSubmit} className="rounded-2xl border border-white/10 bg-white/[0.06] p-6 md:p-8 shadow-2xl backdrop-blur-sm" noValidate>
            {/* Error banner */}
            <ErrorBanner items={errorList} />
            {requestError && (
              <div className="mb-4 rounded-lg border border-red-400/40 bg-red-500/10 p-3 text-red-200" role="alert">
                {requestError}
              </div>
            )}
            ...
            <div className="mb-6">
              ...
              <div className={grid2}>
                <div>
                  ...
                  <input id="customer_name" className={`${baseInput} ${errors.customer_name ? errRing : okRing}`} placeholder="Jane Smith" value={form.customer_name} onChange={(e) => setField("customer_name", e.target.value)} />
                  {errors.customer_name && <p className="mt-1 text-xs text-red-300">{errors.customer_name}</p>}
                </div>
                <div>
                  <label className={label} htmlFor="email">Email *</label>
                  <input id="email" type="email" className={`${baseInput} ${errors.email ? errRing : okRing}`} placeholder="name@company.com" value={form.email} onChange={(e) => setField("email", e.target.value)} />
                  {errors.email && <p className="mt-1 text-xs text-red-300">{errors.email}</p>}
                </div>
                <div>
                  <label className={label} htmlFor="phone">Phone *</label>
                  <input id="phone" className={`${baseInput} ${errors.phone ? errRing : okRing}`} placeholder="+44 7…" value={form.phone} onChange={(e) => setField("phone", e.target.value)} />
                ...
                <div>
                  ...
                  <select id="customer_type" className={`${baseInput} ${okRing}`} value={form.customer_type} onChange={(e) => setField("customer_type", e.target.value as any)}>
                ...
                {form.customer_type === "business" && (
                  <div className="md:col-span-2">
                    ...
                    <input id="company_name" className={`${baseInput} ${okRing}`} value={form.company_name} onChange={(e) => setField("company_name", e.target.value)} />
                  </div>
                )}
              </div>
            </div>
            ...
            <div className="mb-6">
              ...
              <div className={grid2}>
                <div>
                  ...
                  <input id="postcode" className={`${baseInput} ${errors.postcode ? errRing : okRing}`} placeholder="SW1A 1AA" value={form.postcode} onChange={(e) => setField("postcode", e.target.value)} />
                  {errors.postcode && <p className="mt-1 text-xs text-red-300">{errors.postcode}</p>}
                </div>
                <div>
                  <label className={label} htmlFor="city">City/Town</label>
                  <input id="city" className={`${baseInput} ${okRing}`} placeholder="London" value={form.city} onChange={(e) => setField("city", e.target.value)} />
                </div>
              </div>
            </div>
            ...
            <div className="mb-6">
              ...
              <div className={grid2}>
                <div>
                  ...
                  <select id="fuel" className={`${baseInput} ${okRing}`} value={form.fuel} onChange={(e) => setField("fuel", e.target.value as any)}>
                    <option value="diesel">Diesel</option>
                    <option value="petrol">Petrol</option>
                  </select>
                </div>
                <div>
                  <label className={label} htmlFor="quantity_litres">Quantity (litres) *</label>
                  <input id="quantity_litres" type="number" min={1} step="1" className={`${baseInput} ${errors.quantity_litres ? errRing : okRing}`} placeholder="1000" value={form.quantity_litres} onChange={(e) => setField("quantity_litres", e.target.value)} />
                  {errors.quantity_litres && <p className="mt-1 text-xs text-red-300">{errors.quantity_litres}</p>}
                </div>
                <div>
                  <label className={label} htmlFor="urgency">Urgency</label>
                  <select id="urgency" className={`${baseInput} ${okRing}`} value={form.urgency} onChange={(e) => setField("urgency", e.target.value as any)}>
                ...
                <div>
                  <label className={label} htmlFor="preferred_delivery">Preferred delivery date</label>
                  <input id="preferred_delivery" type="date" className={`${baseInput} ${okRing}`} value={form.preferred_delivery} onChange={(e) => setField("preferred_delivery", e.target.value)} />
                </div>
              </div>
            </div>
            ...
            <div className={grid2}>
              <div>
                <label className={label} htmlFor="use_case">Use case</label>
                <input id="use_case" className={`${baseInput} ${okRing}`} placeholder="vehicles, machinery, generators…" value={form.use_case} onChange={(e) => setField("use_case", e.target.value)} />
              </div>
              <div>
                <label className={label} htmlFor="access_notes">Access notes</label>
                <input id="access_notes" className={`${baseInput} ${okRing}`} placeholder="e.g., gate code 1234, height limit 3.5m" value={form.access_notes} onChange={(e) => setField("access_notes", e.target.value)} />
              </div>
              <div className="md:col-span-2">
                <label className={label} htmlFor="notes">Notes</label>
                <textarea id="notes" rows={3} className={`${baseInput} ${okRing}`} placeholder="Anything else we should know?" value={form.notes} onChange={(e) => setField("notes", e.target.value)} />
              </div>
            </div>

            <label className="mt-4 flex items-center gap-3 text-sm">
              <input type="checkbox" className="accent-yellow-500" checked={form.marketing_opt_in} onChange={(e) => setField("marketing_opt_in", e.target.checked)} />
            ...
            <div className="mt-5 space-y-4">
              ...
              {captchaError && <p className="text-sm text-red-300">{captchaError} Please refresh and try again.</p>}
              ...
              <p className="text-xs text-white/70 text-center">
                ...
                Service subject to credit checks, site survey and safety compliance. Protected by hCaptcha.
              </p>
            </div>
          </form>
        )}
      ...
      <Modal open={showAbout} onClose={() => setShowAbout(false)} title="Who we are">
        <div className="space-y-3 text-white/90">
          <p>FuelFlow helps businesses and residential customers reduce their ongoing fuel cost with live pricing, simple ordering and reliable delivery partners.</p>
          ...
          <a href="https://fuelflow.co.uk/fuel-tank-options" target="_blank" rel="noreferrer" className="inline-block mt-2 text-yellow-300 hover:underline">
      ...
      <Modal open={showCalc} onClose={() => setShowCalc(false)} title="Savings calculator">
        <div className={grid2}>
          <div>
            ...
            <input type="number" min={0} className={`${baseInput} ${okRing}`} value={consumption} onChange={(e) => setConsumption(Number(e.target.value))} />
          </div>
          <div>
            <label className={label}>Market price (£/L)</label>
            <input type="number" step="0.01" min={0} className={`${baseInput} ${okRing}`} value={marketPrice} onChange={(e) => setMarketPrice(Number(e.target.value))} />
          </div>
          <div>
            <label className={label}>FuelFlow price (£/L)</label>
            <input type="number" step="0.01" min={0} className={`${baseInput} ${okRing}`} value={ourPrice} onChange={(e) => setOurPrice(Number(e.target.value))} />
          </div>
          <div>
            <label className={label}>Model</label>
            <select className={`${baseInput} ${okRing}`} value={option} onChange={(e) => setOption(e.target.value as "rent" | "buy")}>
          ...
          {option === "buy" && (
            <div className="md:col-span-2">
              ...
              <input type="number" min={0} className={`${baseInput} ${okRing}`} value={tankPrice} onChange={(e) => setTankPrice(Number(e.target.value))} />
            </div>
          )}
        </div>
        ...
        {option === "buy" && (
          <div className="mt-4 rounded-lg bg-white/[0.06] border border-white/10 p-4">
            <div className="text-sm text-white/85">
              Estimated payback: <strong>{isFinite(paybackMonths) ? `${paybackMonths.toFixed(1)} months` : "—"}</strong>
        ...
        <p className="mt-5 text-xs text-white/70">
          Calculations are indicative only and may exclude delivery/installation. Pricing is dynamic and confirmed at order. Any free-rental model requires minimum volumes, credit approval and adherence to service terms.
        </p>

        <div className="mt-5 flex justify-end">
          <button onClick={() => setShowCalc(false)} className="rounded-lg bg-white/10 px-4 py-2 hover:bg-white/15">
            Close
          </button>
        </div>
      </Modal>
      ...
      {/* Terms required modal */}
      <Modal open={showTermsRequired} onClose={() => setShowTermsRequired(false)} title="Please accept Terms & Conditions">
        <p className="text-white/85">
          You need to review and accept our latest Terms & Conditions before submitting a request.
        </p>
        <div className="mt-4 flex flex-col sm:flex-row gap-3">
          <a
            ...
            target="_blank"
            rel="noreferrer"
            className="inline-flex justify-center rounded-xl bg-yellow-500 px-4 py-2 font-semibold text-[#041F3E] hover:bg-yellow-400"
          >
            Open Terms
          </a>
          <button
            onClick={() => {
              // User may have accepted in the other tab — try submission again
              setShowTermsRequired(false);
            }}
            className="inline-flex justify-center rounded-xl bg-white/10 px-4 py-2 hover:bg-white/15"
          >
            I’ve accepted — retry
          </button>
        </div>
        <p className="mt-3 text-xs text-white/60">
          The Accept button on the Terms page enables only after you scroll near the end and pass hCaptcha.
        </p>
      </Modal>
    </div>
  );
