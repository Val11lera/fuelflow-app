...
export default function QuotePage() {
  ...
  const input = "w-full p-2 rounded border border-white/20 bg-white/5 focus:outline-none focus:ring focus:ring-yellow-500/30";
  ...
  return (
    <div className="min-h-screen flex items-center justify-center p-6 bg-[#041F3E] text-white">
      <div className="w-full max-w-2xl">
        ...
        <p className="mb-6 text-white/80">For non-registered customers. Registered users can order at live prices in the dashboard.</p>
        ...
        <form onSubmit={onSubmit} className="bg-[#0E2E57] p-6 md:p-8 rounded-2xl shadow-xl space-y-6">
          ...
          <div>
            ...
            <div className={grid}>
              ...
              {form.customer_type === "business" && (
                <div className="md:col-span-2"><label className={label}>Company name</label>
          ...
          <div>
            ...
            <div className={grid}>
              <div><label className={label}>Postcode *</label><input className={input} value={form.postcode} onChange={(e)=>setForm({...form, postcode:e.target.value})}/></div>
              <div><label className={label}>City/Town</label><input className={input} value={form.city} onChange={(e)=>setForm({...form, city:e.target.value})}/></div>
            </div>
          </div>
          ...
          <div>
            ...
            <div className={grid}>
              ...
              <div><label className={label}>Quantity (litres) *</label>
                <input type="number" min={1} step="1" className={input} value={form.quantity_litres} onChange={(e)=>setForm({...form, quantity_litres:e.target.value})}/>
              </div>
              ...
              <div><label className={label}>Preferred delivery date</label>
          ...
          {/* Extras */}
          <div className={grid}>
            <div><label className={label}>Use case</label><input className={input} placeholder="vehicles, machinery, generators…" value={form.use_case} onChange={(e)=>setForm({...form, use_case:e.target.value})}/></div>
            <div><label className={label}>Access notes</label><input className={input} placeholder="e.g., gate code 1234, height limit 3.5m" value={form.access_notes} onChange={(e)=>setForm({...form, access_notes:e.target.value})}/></div>
            <div className="md:col-span-2"><label className={label}>Notes</label><textarea rows={3} className={input} value={form.notes} onChange={(e)=>setForm({...form, notes:e.target.value})}/></div>
          </div>
